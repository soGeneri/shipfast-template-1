...
export default function Assessments() {
  ...
  useEffect(() => {
    ...
    const getAssessmentsByStudentId = async () => {
      ...
      if (error) {
        toast.error(
          "Erro ao buscar avaliações por aluno. Entre em contato com o suporte."
        );
      }
  ...
  const handleAccordion = (studentId: string) => {
    const assessmentsForStudent = assessmentsByStudentId.filter(
  ...
  const renderAssessmentDetails = (assessments: any[]) => {
    ...
    const renderBodyMeasurement = (bodyMeasurement: any) => {
      const translatedBodyMeasurements = Object.entries(bodyMeasurement)
        .filter(([key]) =>
          ["hip", "chest", "neck", "waist", "abdomen", "shoulder"].includes(key)
        )
        .map(([key, value]) => (
          <li key={key}>{`${translateKey(
            key,
            bodyMeasurementTranslations
          )}: ${value}`}</li>
        ));
      ...
      const renderSideMeasurements = (side: "right" | "left") => {
        return (
          <div>
            ...
            <ul>
              {Object.entries(bodyMeasurement[side]).map(
                ([innerKey, innerValue]) => (
                  <li key={innerKey}>{`${translateKey(
                    innerKey,
                    bodyMeasurementTranslations
                  )}: ${innerValue}`}</li>
                )
        ...
        <div className="accordion-item">
          <div className="accordion-body space-y-4">
            <div className="flex flex-col md:flex-row">
              <div className="flex-1">
                ...
                <p>
                  ...
                  {assessment.assessment_type === "pollock_7"
                    ? "Pollock de 7 dobras"
                    : "Pollock de 3 dobras"}
                </p>
              </div>
            ...
            <div className="flex flex-col md:flex-row">
              <div className="flex-1">
                <p>
                  <strong>Medidas da Avaliação:</strong>
                </p>
                <ul>
                  {Object.entries(assessment.assessment_measures).map(
                    ([key, value]) => (
                      <li key={key}>{`${translateKey(
                        key,
                        assessmentMeasureTranslations
                      )}: ${value}`}</li>
                    )
                  )}
                </ul>
              ...
              <div className="flex-1">
                ...
                <ul>
                  {Object.entries(assessment.assessment_result).map(
                    ([key, value]) => {
  ...
  return (
    <Layout>
      ...
      <button
        className="btn hover:bg-indigo-600 hover:text-white mt-8"
          ...
          <tbody>
            {students.map((student) => (
              <>
                <tr
                  key={student.id}
                  onClick={() => handleAccordion(student.id)}
